...
from global_state import _update_real_time_plots
...
def estimate_remaining_steps(wid, state):
    ...
    template = GlobalConfig.path_templates["A"]
    try:
...
def run_simulation_a1():
    ...
    try:
        ...
        #     t = Thread(target=wafer_process,
        #                args=(wid, matid, lpname, path_type, state, scheduler))
        #     t.start()
        #     threads.append(t)
        #     time.sleep(random.uniform(0.01, 0.05) * GlobalConfig.TIME_SCALE)
        ...
        if GlobalConfig.PLOT_ENABLED:
            # 保存ACO参数进化图
            scheduler.aco.save_adaptation_report("aco_parameters_1a.png")
            # 保存实时监控图
            fig.savefig("real_time_monitor_1a.png", dpi=300)
            plt.close(fig)
        # ================= 4. 清理资源 =================
        print("\n 正在停止所有线程...")
