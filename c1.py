...
plt.rcParams['font.sans-serif'] = ['Microsoft YaHei', 'SimHei', 'Arial Unicode MS']  # 中文字体优先级
...
def estimate_remaining_steps(wid, state):
    ...
    # 基于路径模板的剩余步骤计算
    if path_type == "C":
        template = ["LLA", ["PM7", "PM8"], "LLC", ["PM1", "PM2", "PM3", "PM4"], "LLD", ["PM9", "PM10"], "LLA"]
    else:
        template = ["LLA", ["PM7", "PM8"], ["PM9", "PM10"], "LLD", "LLA"]
...
def run_simulation_c1():
    ...
    try:
        ...
        # 任务一(c)配置：LP1使用路径C，LP2和LP3使用路径D
        for lp_index in range(1, GlobalConfig.LP_COUNT + 1):
            for wafer_in_lp in range(1, GlobalConfig.LP_WAFER_COUNT + 1):
                ...
                path_type = "C" if lp_index == 1 else "D"  # LP1用路径C，其他用D
        ...
        if GlobalConfig.PLOT_ENABLED:
            # 保存ACO参数进化图
            scheduler.aco.save_adaptation_report("aco_parameters_1c.png")
            # 保存实时监控图
            fig.savefig("real_time_monitor_1c.png", dpi=300)
            plt.close(fig)
        # ================= 4. 清理资源 =================
        print("\n 正在停止所有线程...")
        ...
        # 保存调度结果
        with open("c1_json.json", "w") as f:
            json.dump({"MoveList": state.move_list}, f, indent=4)

        # 绘制甘特图
        draw_gantt(state.gantt_data, "c")
        # 绘制多目标优化趋势图
