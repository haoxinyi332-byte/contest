...
class GlobalConfig:
    ...
    PROCESS_TIMES = {
        ...
        "PM1": 100, "PM2": 100, "PM3": 200, "PM4": 200, "PM5": 200, "PM6": 200
    }
    # 修改清洁条件及时间（任务2a）
    CLEAN_CONDITIONS = {
        "idle": {"threshold": 100, "time": 50},
        "path_change": {"time": 150},
        "wafer_count": {"threshold": 3, "time": 200}
    }
    # 各种时间参数（任务2）
    PUMP_TIME = 15
    VENT_TIME = 20
    ...
    TIME_SCALE = 0.001
    # JIT驻留时间限制
    JIT_PM_STAY_LIMIT = 15
    JIT_TRANSFER_LIMIT = 30
    # LP数量和每盒晶圆数量
    LP_COUNT = 3
    LP_WAFER_COUNT = 25
    ...
    PLOT_UPDATE_INTERVAL = 20  # 绘图更新间隔（次）
    PLOT_TIME_INTERVAL = 10.0  # 绘图更新间隔（秒）
    # 信息素挥发率
    ...
    path_templates = {
        # 2a
        "L": [["LLA", "LLB"], ["PM7", "PM8"], "LLC", "PM3", "LLD", "PM1", "LLD2", "PM10", "LLD2", "LLA"],
        "M": [["LLA", "LLB"], ["PM7", "PM8"], ["PM9", "PM10"], "LLD2", "LLA"],
        "N": [["LLA", "LLB"], "PM7", "LLC", "PM3", "LLD", "PM8", "LLC", "PM1", "LLD", "PM8", "LLA"],
        # 2b
        "O":[["LLA", "LLB"], "PM7", "LLC", "PM4", "LLD", "PM8", "LLC", "PM1", "LLD", "PM8", "LLA"],
        "P":[["LLA", "LLB"], "PM7", "LLC", "PM5", "LLD", "PM8", "LLC", "PM1", "LLD", "PM8", "LLA"],
        "Q":[["LLA", "LLB"], ["PM7", "PM8"], "LLC", "PM3", "LLD", "PM1", "LLD2", "PM10", "LLD2", "LLA"],
        # 2c
        "R": [["LLA", "LLB"], ["PM7", "PM8"], "LLC", ["PM1", "PM2"], "LLD2", "LLA"],
        "S":[["LLA", "LLB"], ["PM7", "PM8"], "LLC", "PM1", "LLD2", "LLA"],
        "T":[["LLA", "LLB"], ["PM7", "PM8"], "LLC", "PM2", "LLD2", "LLA"],
        # 2d
        "U":["LLA"] + ["PM7", "PM8", "LLC", "LLD2", "LLB"] * 3,
        "V": [["LLA", "LLB"], ["PM7", "PM8"], "LLC", ["PM1", "PM2", "PM3"], "LLD2", "LLA"],
        "W":[["LLA", "LLB"], ["PM7", "PM8"], "LLC", ["PM4", "PM5", "PM6"], "LLD2", "LLA"],
    }
    WAFER_WORKERS = 75
# ================= 系统状态 =================
class SystemState:
    def __init__(self):
        ...
        self.semaphore_status_interval = 100  # 100秒无变化时打印信号量状态
...
# 紧急恢复：将所有非LP晶圆运回晶圆盒，并基于剩余路径估算完成时间
def emergency_recovery(state):
    ...
    module_times = {
        **{f"PM{i}": GlobalConfig.PROCESS_TIMES[f"PM{i}"] for i in range(1, 11)},
        ...
        "LL_transfer": max(GlobalConfig.PUMP_TIME, GlobalConfig.VENT_TIME)+GlobalConfig.PLACE_TIME_TM2,
        "TM1_transfer": GlobalConfig.TRANSFER_TIME_TM1,
        "cooling": GlobalConfig.COOL_TIME,
        "open_close":GlobalConfig.OPEN_DOOR_TIME*2 +GlobalConfig.PICK_TIME_TM2
    }
    for wid, current_loc in wafers_to_recover:
        ...
        for current_loc2 in remaining_path:
            ...
            if current_loc2 in ["LLA", "LLB"]:
                with state.sem_TM1:
                    log_move("PickMove", "TM1", GlobalConfig.PICK_TIME_TM1, state, matid)
                    log_move("TransferMove", "TM1", GlobalConfig.TRANSFER_TIME_TM1, state, matid)
                    log_move("PlaceMove", "TM1", GlobalConfig.PLACE_TIME_TM1, state, matid)
                ...
                with state.sem_TM2 if current_loc2 not in ["LLC"]+[f"PM{i}" for i in range(1, 7)] else state.sem_TM3:
                    pick_time = GlobalConfig.PICK_TIME_TM2 if current_loc2 not in ["LLC"]+[f"PM{i}" for i in range(1, 7)] else GlobalConfig.PICK_TIME_TM3
                    ...
                    if current_loc2 in ["LLC", "LLD"]+[f"PM{i}" for i in range(1, 7)] and ll_exit in ["LLC", "LLD"]+[f"PM{i}" for i in range(1, 7)]:
                        ...
                        log_move("PlaceMove", "TM3", GlobalConfig.PLACE_TIME_TM3, state, matid, slot=2)
                        ...
                        log_move("PrepareMove",ll_exit,GlobalConfig.OPEN_DOOR_TIME,state,matid,slot=2)
                        log_move("PlaceMove", "TM2", GlobalConfig.PLACE_TIME_TM2, state, matid, slot=2)
        ...
        with state.sem_LP[lp_name]:
            log_move("CompleteMove", lp_name, GlobalConfig.CLOSE_DOOR_TIME, state, matid)
        # 添加运输回LP的时间
        remaining_time += GlobalConfig.PICK_TIME_TM1 + GlobalConfig.PLACE_TIME_TM1
...
def detect_deadlock(state):
    """死锁检测（支持PM清洁/LL状态转换时的资源占用）"""
    graph = defaultdict(set)
    resources = {}

    # ========== 1. PM资源占用 ==========
    for pm, status in state.pm_status.items():
        if status["in_use"] or status["cleaning"]:
            # 若PM正在加工 或 清洁中，都视为被当前晶圆占用
            wafer = status.get("current_wafer") or status.get("last_wafer")
            if wafer:
                resources[f"PM_{pm}"] = wafer
    ...
    for ll in ["LLA", "LLB", "LLC", "LLD"]:
        ...
        # 状态转换占用：若 LL 正在转换状态，也视为资源暂时不可用
        ll_state = state.ll_state.get(ll, "")
        if ll_state in ["vac_to_atm", "atm_to_vac"]:  # 状态正在转换中
            # 标记整个LL（抽象资源）不可用，假设无具体晶圆，可写None
            resources[f"LL_{ll}_LOCKED"] = None
    ...
    for wafer_id, (module, _) in list(state.wafer_current_location.items()):
        next_res = get_next_step_for_wafer(wafer_id, state)
        if not next_res:
            continue
        # 判断是否被其他晶圆占用
        for res_name, holder in resources.items():
            if next_res in res_name:  # 允许模糊匹配资源，如 LL_LLA_ 和 LL_LLA_LOCKED
                if holder is not None and holder != wafer_id:
...
def estimate_remaining_steps(wid, state):
    ...
    path_type = get_wafer_path_type(wid, state)
    # 基于路径模板的剩余步骤计算
    template = GlobalConfig.path_templates[path_type]
    try:
...
def get_wafer_path_type(wid, state):
    for task in state.wafer_tasks:
        if task[0] == wid:
            path_type = task[3]
            if path_type in ["L", "M", "N", "O", "P", "Q", "R", "S", "T", "U", "V", "W"]:
                return path_type
    return "L"
...
def tm2_transfer_wafer(wid, matid, from_mod, to_mod,state):
    ...
    with state.sem_TM2:
        ...
        if opposite:
            # 确保另一手臂没有冲突
            pass
        pick_time = GlobalConfig.PICK_TIME_TM2
        ...
        # 放晶圆时间
        place_time = GlobalConfig.PLACE_TIME_TM2
...
def tm3_transfer_wafer(wid, matid, from_mod, to_mod, state):
    ...
    with state.sem_TM3:
        ...
        # 计算移动时间
        pick_time = GlobalConfig.PICK_TIME_TM3
        ...
        # 放晶圆时间
        dest_slot = 1
        place_time = GlobalConfig.PLACE_TIME_TM3
...
def check_and_clean_pm(pm_name, current_path, move_list, gantt_data, start_time, state):
    ...
    if clean_required and not status["cleaning"]:
        ...
        status["idle_start"] = now + clean_time
...
def log_move(action, module, duration, state, matid,
             ...
             src_slot=None, dest_slot=None):
    """记录移动操作"""
    t_start = None  # 初始化为None，确保变量存在
    try:
        t_start = get_now(state.start_time)
        # 根据不同的机械手类型使用不同的操作时间
        if action == "PickMove":
            if module == "TM1":
                duration = GlobalConfig.PICK_TIME_TM1
            elif module == "TM2":
                duration = GlobalConfig.PICK_TIME_TM2
            elif module == "TM3":
                duration = GlobalConfig.PICK_TIME_TM3
        elif action == "PlaceMove":
            if module == "TM1":
                duration = GlobalConfig.PLACE_TIME_TM1
            elif module == "TM2":
                duration = GlobalConfig.PLACE_TIME_TM2
            elif module == "TM3":
                duration = GlobalConfig.PLACE_TIME_TM3
        ...
        with state.mutex_print:
            state.error_log.append((matid, f"Logging error at {module}: {str(e)}"))
        return t_start + duration if t_start is not None else duration  # 安全回退
...
class DynamicACO:
    def __init__(self, state, num_ants=5, max_iterations=10):
        ...
        self.last_improvement = get_now(self.state.start_time)
        self.ant_paths = {}
        # 历史记录
        self.history = {
            'time': [0.0],
    ...
    # def select_pm(self, candidates, urgency, wid=None):
    #     """
    #     模拟蚂蚁行为的PM选择函数（多蚂蚁探索版）
    #     :param candidates: 候选PM列表
    #     :param urgency: 晶圆紧急度（0~1）
    #     :param wid: 晶圆ID（用于计算当前JIT紧急度）
    #     """
    #     self.adapt_parameters()
    #     now = get_now(self.state.start_time)
    #     num_ants = 5  # 蚂蚁数量（可配置）
    #     ant_scores = []  # 存储每只蚂蚁的探索结果
    #
    #     # --- 每只蚂蚁独立探索 ---
    ...
    #             with self.state.mutex_print:
    #                 status = self.state.pm_status[pm]
    #                 pheromone = self.get_pheromone(pm)
    #
    #                 # ===== 启发函数（带随机扰动模拟蚂蚁感知差异） =====
    #                 pm_in_use = status["in_use"]
    #                 pm_cleaning = status["cleaning"]
    #                 last_end_time = status["last_end_time"]
    #
    #                 # 添加随机扰动（蚂蚁对时间的感知可能有误差）
    #                 perturbed_delay = max(0, (max(last_end_time, now) - now) * random.uniform(0.9, 1.1))
    #
    #                 # 1. 空闲性奖励
    #                 idle_bonus = 1.0 if (not pm_in_use and not pm_cleaning) else 0.1
    #
    ...
    #                 # 3. JIT紧急度（添加蚂蚁个体差异）
    #                 if wid:
    #                     jit_level = calculate_jit_urgency(wid, now, self.state) * random.uniform(0.95, 1.05)
    #                 else:
    #                     jit_level = urgency * random.uniform(0.95, 1.05)
    #
    #                 heuristic = idle_bonus * time_score * (1 + jit_level)
    #
    #                 # ===== 综合分数 =====
    ...
    #         # 当前蚂蚁的选择（概率选择或直接选最优）
    #         if total > 0:
    #             if random.random() < 0.8:  # 80%概率按信息素选择
    #                 probs = [s / total for _, s in scores]
    #                 chosen_pm = np.random.choice([pm for pm, _ in scores], p=probs)
    #             else:  # 20%概率随机探索（模拟蚂蚁的探索行为）
    #                 chosen_pm = random.choice(candidates)
    #             ant_scores.append((chosen_pm, sum(s for _, s in scores)))
    #
    #     # --- 蚁群决策：选择综合评分最高的PM ---
    #     if ant_scores:
    #         # 按(PM, 总分数)排序
    #         ant_scores.sort(key=lambda x: -x[1])
    #         best_pm = ant_scores[0][0]
    #
    #         # 更新信息素（奖励被选中的PM）
    #         self.update_pheromone(best_pm)
    #         return best_pm
...
def get_next_module(current_module, path_template):
    """根据当前模块和路径模板返回下一个模块"""
    for i, module in enumerate(path_template):
        # 处理嵌套列表（如["PM1","PM2"]）
        if isinstance(module, list):
            if current_module in module:
                next_idx = i + 1
                if next_idx < len(path_template):
                    return path_template[next_idx]
                return None  # 已经是最后一个模块
        # 处理普通模块（如"LLC"）
        elif module == current_module:
            next_idx = i + 1
            if next_idx < len(path_template):
                next_module = path_template[next_idx]
                # 如果下一个模块是列表，则随机选择一个（或按策略选择）
                if isinstance(next_module, list):
                    return random.choice(next_module)
                return next_module
            return None  # 已经是最后一个模块
    raise ValueError(f"当前模块 {current_module} 不在路径模板中")
...
def wafer_process(wid, matid, lp_name, path_type, state, scheduler):
    ...
    with state.lp_sequence_lock[lp_name]:
        ...
        with state.sem_LP[lp_name]:
            ...
            with state.sem_TM1:
                log_move("PickMove", "TM1", GlobalConfig.PICK_TIME_TM1, state, matid,src_station=lp_name,dest_station="TM1",src_slot=1,dest_slot=1)
    ...
    log_move("PrepareMove", ll_start, GlobalConfig.OPEN_DOOR_TIME, state, matid, slot=1, src_station=ll_start,dest_station=ll_start,src_slot=1,dest_slot=1)
    with state.sem_LL[ll_start]:
        log_move("PlaceMove", ll_start, GlobalConfig.PLACE_TIME_TM1, state, matid, slot=2,src_station=ll_start,dest_station=ll_start,src_slot=1,dest_slot=2)
    log_move("CompleteMove", ll_start, GlobalConfig.CLOSE_DOOR_TIME, state, matid, slot=2,src_station=ll_start,dest_station=ll_start,src_slot=2,dest_slot=2)
    ...
    for i in range(1, len(path) - 1):  # 跳过第一个和最后一个LL
        ...
        if isinstance(current_module, list) or (isinstance(current_module,str) and current_module.startswith("PM")):
            ...
            with state.sem_PM[chosen_pm]:
                ...
                end_time = pm_process_with_jit(chosen_pm, wid, ptime,matid, state,src_station=chosen_pm,dest_station=chosen_pm,src_slot=1,dest_slot=1)
                # 更新PM状态
                ...
                # leave_pm_queue(chosen_pm, wid, state)
            ...
            log_move("PrepareMove", current_module, GlobalConfig.OPEN_DOOR_TIME, state, matid, slot=2, src_station=current_module,dest_station=current_module,src_slot=2,dest_slot=2)
            with state.sem_LL[current_module]:
                log_move("PlaceMove", current_module, GlobalConfig.PLACE_TIME_TM1, state, matid, slot=2, src_station=last,dest_station=current_module,src_slot=1,dest_slot=2)
            log_move("CompleteMove", current_module, GlobalConfig.CLOSE_DOOR_TIME, state, matid, slot=2, src_station=current_module,dest_station=current_module,src_slot=2,dest_slot=2)
            ...
            if current_module == "LLD2":
                ...
                log_move("PrepareMove", "LLD", GlobalConfig.OPEN_DOOR_TIME, state, matid, slot=1, src_station="LLD",dest_station="LLD", src_slot=1,dest_slot=1)
                with state.sem_LL["LLD"]:
                    log_move("PlaceMove", "LLD", GlobalConfig.PLACE_TIME_TM1, state, matid, slot=1, src_station=last,dest_station="LLD", src_slot=1, dest_slot=1)
                log_move("ProcessMove", "LLD", GlobalConfig.COOL_TIME, state, matid, src_station="LLD",dest_station="LLD",src_slot=1,dest_slot=1)
                log_move("CompleteMove", "LLD", GlobalConfig.CLOSE_DOOR_TIME, state, matid, slot=1, src_station="LLD",dest_station="LLD",src_slot=1,dest_slot=1)
                ...
                log_move("PrepareMove", current_module, GlobalConfig.OPEN_DOOR_TIME, state, matid, slot=1, src_station=current_module,dest_station=current_module, src_slot=1,dest_slot=1)
                with state.sem_LL[current_module]:
                    log_move("PlaceMove", current_module, GlobalConfig.PLACE_TIME_TM1, state, matid, slot=1, src_station=last,dest_station=current_module, src_slot=1,dest_slot=1)
                log_move("CompleteMove", current_module, GlobalConfig.CLOSE_DOOR_TIME, state, matid, slot=1, src_station=current_module,dest_station=current_module, src_slot=1,dest_slot=1)
                update_wafer_location(wid, current_module, get_now(state.start_time), state)
                wafer["current_stage"] = current_module
                end_time = get_now(state.start_time)
                wafer["last_move_time"] = end_time
        ...
        with state.sem_TM2:
            ...
            if last in ["LLC", "LLD"]:
                with state.ll_slot_lock[last]:
                    state.ll_slots[last]["S1"] = None
                    state.ll_slots[last]["S2"] = None
            log_move("PickMove", wafer["current_stage"], GlobalConfig.PICK_TIME_TM2, state, matid, slot=1,src_station=last,dest_station="TM2",src_slot=1,dest_slot=1)
            ...
            log_move("TransferMove", "TM2", move_time, state, matid, slot=2,src_station=wafer["current_stage"], dest_station=ll_exit,src_slot=1,dest_slot=2)

            # 放入LLA/LLB的S2槽
            log_move("PrepareMove", ll_exit, GlobalConfig.OPEN_DOOR_TIME, state, matid, slot=2, src_station=ll_exit,dest_station=ll_exit, src_slot=2,dest_slot=2)
            ll_put_into_S2(ll_exit, wid, state)
            ll_move_S2_to_S1(ll_exit, state)
            with state.sem_LL[ll_exit]:
                log_move("PlaceMove", ll_exit, GlobalConfig.PLACE_TIME_TM2, state, matid, slot=2,src_station="TM2",dest_station=ll_exit,src_slot=1,dest_slot=2)
            log_move("CompleteMove", ll_exit, GlobalConfig.CLOSE_DOOR_TIME, state, matid, slot=2,src_station=ll_exit,dest_station=ll_exit,src_slot=2, dest_slot=2)

            update_wafer_location(wid, ll_exit, get_now(state.start_time), state)
    # 3. 从LLA/LLB到LP（使用TM1大气机械手）
    with state.sem_TM1:
        log_move("PickMove", "TM1", GlobalConfig.PICK_TIME_TM1, state, matid,src_station=ll_exit,dest_station="TM1",src_slot=1,dest_slot=1)
        log_move("TransferMove", "TM1", GlobalConfig.TRANSFER_TIME_TM1, state, matid,src_station=ll_exit,dest_station=lp_name,src_slot=1,dest_slot=1)
        log_move("PlaceMove", "TM1", GlobalConfig.PLACE_TIME_TM1, state, matid,src_station="TM1",dest_station=lp_name,src_slot=1,dest_slot=1)
    # 4. 放回LP
    with state.sem_LP[lp_name]:
        log_move("CompleteMove", lp_name, GlobalConfig.CLOSE_DOOR_TIME, state, matid,src_station=lp_name,dest_station=lp_name,src_slot=1,dest_slot=1)
    # 更新最终位置
...
def run_simulate(task):
    ...
    try:
        ...
        wafer_tasks = []
        wafer_id = 1
        # 任务二(d)配置：LP1使用路径U，LP2使用路径V，LP3使用路径W
        for lp_index in range(1, GlobalConfig.LP_COUNT + 1):
            for wafer_in_lp in range(1, GlobalConfig.LP_WAFER_COUNT + 1):
                ...
                if task == "a":
                    if lp_index == 1:
                        path_type = "L"
                        GlobalConfig.PROCESS_TIMES["PM1"] = 200
                    elif lp_index == 2:
                        path_type = "M"
                    else:
                        path_type = "N"
                        GlobalConfig.PROCESS_TIMES["PM1"] = 100
                elif task == "b":
                    if lp_index == 1:
                        path_type = "O"
                        GlobalConfig.PROCESS_TIMES["PM7"] = 70
                        GlobalConfig.PROCESS_TIMES["PM8"] = 70
                        GlobalConfig.PROCESS_TIMES["PM1"] = 100
                    elif lp_index == 2:
                        path_type = "P"
                        GlobalConfig.PROCESS_TIMES["PM7"] = 70
                        ...
                        path_type = "Q"
                        GlobalConfig.PROCESS_TIMES["PM7"] = 200
                        GlobalConfig.PROCESS_TIMES["PM8"] = 200
                        GlobalConfig.PROCESS_TIMES["PM1"] = 200
                elif task == "c":
                    GlobalConfig.PROCESS_TIMES["PM1"] = 300
                    GlobalConfig.PROCESS_TIMES["PM2"] = 300
                    if lp_index == 1:
                        path_type = "R"
                    elif lp_index == 2:
                        path_type = "S"
                    ...
                    GlobalConfig.PROCESS_TIMES["PM1"] = 600
                    GlobalConfig.PROCESS_TIMES["PM2"] = 600
                    GlobalConfig.PROCESS_TIMES["PM3"] = 600
                    GlobalConfig.PROCESS_TIMES["PM4"] = 600
                    GlobalConfig.PROCESS_TIMES["PM5"] = 600
                    GlobalConfig.PROCESS_TIMES["PM6"] = 600
                    if lp_index == 1:
                        path_type = "U"
                    elif lp_index == 2:
                        path_type = "V"
        ...
        with ThreadPoolExecutor(max_workers=GlobalConfig.WAFER_WORKERS) as executor:
            ...
            while any(not f.done() for f in futures):
                ...
                if (GlobalConfig.PLOT_ENABLED
                    ...
                    _update_real_time_plots(axs, state, scheduler.aco)
                    last_plot_time = time.time()
                # 定期检查死锁
        ...
        if GlobalConfig.PLOT_ENABLED:
            # 保存ACO参数进化图
            if task == "a":
                scheduler.aco.save_adaptation_report("aco_parameters_2a.png")
                # 保存实时监控图
                fig.savefig("real_time_monitor_2a.png", dpi=300)
            elif task == "b":
                scheduler.aco.save_adaptation_report("aco_parameters_2b.png")
                # 保存实时监控图
                fig.savefig("real_time_monitor_2b.png", dpi=300)
            elif task == "c":
                scheduler.aco.save_adaptation_report("aco_parameters_2c.png")
                # 保存实时监控图
                fig.savefig("real_time_monitor_2c.png", dpi=300)
            else:
                scheduler.aco.save_adaptation_report("aco_parameters_2d.png")
                # 保存实时监控图
                fig.savefig("real_time_monitor_2d.png", dpi=300)
            plt.close(fig)
        # ================= 4. 清理资源 =================
        print("\n 正在停止所有线程...")
        state.scheduler_active = False
        ...
        # 保存调度结果
        if task == "a":
            with open("a2_json.json", "w") as f:
                json.dump({"MoveList": state.move_list}, f, indent=4)
        elif task == "b":
            with open("b2_json.json", "w") as f:
                json.dump({"MoveList": state.move_list}, f, indent=4)
        elif task == "c":
            with open("c2_json.json", "w") as f:
                json.dump({"MoveList": state.move_list}, f, indent=4)
        else: # task == "d"
            with open("d2_json.json", "w") as f:
                json.dump({"MoveList": state.move_list}, f, indent=4)
        # 绘制甘特图
        draw_gantt(state.gantt_data,task)
        # 绘制多目标优化趋势图
...
def plot_optimization_trend(history_total_time, history_throughput, history_jit_violation):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("optimization_trend_2.png")
    plt.show()
...
def draw_gantt(gantt_data,task):
    ...
    plt.tight_layout()
    if task == "a":
        plt.savefig("gantt_chart_2a.png")
    elif task == "b":
        plt.savefig("gantt_chart_2b.png")
    elif task == "c":
        plt.savefig("gantt_chart_2c.png")
    else:
        plt.savefig("gantt_chart_2d.png")
    plt.show()
