...
def get_wafer_path_type(wid, state):
    ...
    for task in state.wafer_tasks:
        if task[0] == wid:  # 假设task[0]是晶圆ID
            path_type = task[3]  # 假设task[3]存储路径类型
            # 确保返回的路径类型在定义的范围内（A-J）
            if path_type in ["E", "F", "G", "H", "I", "J", "K"]:
                return path_type
            else:
                print(f"Warning: Unknown path type '{path_type}' for wafer {wid}. Using default 'C'.")
                return "E"  # 无效路径类型时返回默认值

    # 若未找到匹配的晶圆任务，返回默认路径
    print(f"Warning: No task found for wafer {wid}. Using default path 'E'.")
    return "E"
...
def run_simulation_d1():
    ...
    try:
        ...
        wafer_tasks = []
        wafer_id = 1
        # LP1: 1-10用路径E, 11-25用路径F
        for wafer_in_lp in range(1, GlobalConfig.LP_WAFER_COUNT + 1):
            matid = f"1.{wafer_in_lp}"
            lp_name = "LP1"
            path_type = "E" if wafer_in_lp <= 10 else "F"
            wafer_tasks.append((wafer_id, matid, lp_name, path_type))
            wafer_id += 1

            # LP2: 1-5用路径G, 6-15用路径H, 16-25用路径I
        for wafer_in_lp in range(1, GlobalConfig.LP_WAFER_COUNT + 1):
            matid = f"2.{wafer_in_lp}"
            lp_name = "LP2"
            if wafer_in_lp <= 5:
                path_type = "G"
            elif wafer_in_lp <= 15:
            ...
            # LP3: 1-15用路径J, 16-25用路径K
        for wafer_in_lp in range(1, GlobalConfig.LP_WAFER_COUNT + 1):
            matid = f"3.{wafer_in_lp}"
            lp_name = "LP3"
            path_type = "J" if wafer_in_lp <= 15 else "K"
            wafer_tasks.append((wafer_id, matid, lp_name, path_type))
            wafer_id += 1
        ...
        if GlobalConfig.PLOT_ENABLED:
            # 保存ACO参数进化图
            scheduler.aco.save_adaptation_report("aco_parameters_1d.png")
            # 保存实时监控图
            fig.savefig("real_time_monitor_1d.png", dpi=300)
            plt.close(fig)
        # ================= 4. 清理资源 =================
        print("\n 正在停止所有线程...")
        ...
        # 保存调度结果
        with open("d1_json.json", "w") as f:
            json.dump({"MoveList": state.move_list}, f, indent=4)
        # 绘制甘特图
        draw_gantt(state.gantt_data, "d")
        # 绘制多目标优化趋势图
